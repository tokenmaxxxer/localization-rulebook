#!/usr/bin/env bash
# PreToolUse gate (Write|Edit|MultiEdit) for localization-proposal-gate.
...
# methodology-gate.sh — not vendored, rewritten for this surface.
#
# Kill switch: export LOCALIZATION_PROPOSAL_GATE_OFF=1
set -uo pipefail

deny() { echo "localization-proposal-gate: refused — $1" >&2; exit 2; }

case "${LOCALIZATION_PROPOSAL_GATE_OFF:-}" in
  ""|0|false|no|off) ;;
  *) exit 0 ;;
esac
...
MG_PAYLOAD="$payload" MG_ROOT="$root" \
python3 <<'PY'
...
try:
    ...
    try:
        ...
        deny("the tool-call payload is not valid JSON; cannot judge proposal structure on an unparseable write.")
    ...
    if missing:
        deny(
            "proposal is missing required section(s): %s. Per issue-1 (a), a "
            "localization phase-1 proposal must contain all 4 sections — "
            "조사 근거 / 채택 항목 / 논리적 근거 / 반영 계획." % ", ".join(missing)
        )
